...
```text
0.0035 元/月 × 3/30 ≈ 0.00035 元
```

如果每天处理 10 小时音频，并且每份音频保留 3 天，常驻存量约 30 小时音频，存储费约：

```text
30 × 0.0288 GB × 0.12 元/GB/月 ≈ 0.104 元/月
```

因此，保留 1-3 天几乎不会改变总成本。OSS 在这个工作流里不是主要成本，主要成本仍然是 ASR 和 LLM 后处理。
